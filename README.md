...
### 8. ООП и Java
В package model созданы классы, согласно диаграмме п. 6.
### 9. Программа-реестр домашних животных
Программа создана по схеме "model-view-controller". 
Из главного класса MainProgram вызывается класс MenuController, в котором осуществляется 
выбор действия, которое необходимо выполнить.

Добавление и удаление животных из списка осуществляется с применением паттерна 
Factory method в package factory. Здесь же осуществляется и создание даты рождения 
животно при его создании.

Вывод списка животных на экран, вывод списка комманд определенного животного, 
вывод счётчика животных осуществляется в package view.

Обучение животных новым коммандам осуществляется добавлением новых команд в ArrayList.

В package calendar осуществляется проверка корректности заполнения даты рождения животного 
(учитывается количество дней в месяце, високосный год).

В package choice осуществляется выбор животного по номеру (для обучения новым коммандам, 
вывода комманд на экран, удаления из списка).

В package comparator осуществляется сравнивание животных в списке по дате рождения.

В package enumeration находятся перечисления типов животных и режимов работы программы из 
главного меню.

В package iterator осуществляется итерация в списке животных, и в списке комманд животного.

В классе AnimalLogger, что в package logger осуществляется журналирование всей программы.

В package service осуществляется сортировка животных в списке. После сортировки по дате рождения, сохраняется ArrayList в классе AnimalGroupController, но не в AnimalGroup. Это сделано для упрощения сортировки по номеру. При сортировке по номеру просто вызывается список 
из AnimalGroup.